...
#   - refuses to run while Deadlock is up (its cloud would re-clobber, and the
...
if [ "${FORCE:-0}" != "1" ] && "$TASKLIST" 2>/dev/null | grep -qi deadlock; then
    die "Deadlock is running. Exit it fully (tray icon -> Exit), then re-run. (FORCE=1 overrides.)"
fi
...
cp "$SRC" "$LIVE"
touch "$LIVE"
echo "restore: OK ($counts) -> live file. Now start Deadlock so it uploads the good copy."
